# Enhanced Swing Strategy Backtest (Corrected Version)
# ---------------------------------------------------------------------
# Includes:
# - R:R filter
# - Dynamic ATR-based stop
# - Multi-timeframe trend alignment
# - AI-enhanced scoring (placeholder)
# - Macro regime overlay support
# ---------------------------------------------------------------------
...
def ai_score(features: Dict[str, float]) -> float:
    score = (
        0.4 * features.get('breakout_ratio', 1.0) +
        0.3 * features.get('adx14', 20) / 25.0 +
        0.2 * features.get('rsi14', 50) / 50.0 +
...
def backtest(
    ...
    for dt in dates:
        for sym in list(positions.keys()):
            ...
            if exit_reason:
                trades.append(dict(date=dt, symbol=sym, side='SELL', qty=pos.qty, price=round(c,2), reason=exit_reason))
                cash += pos.qty * c
                del positions[sym]
        ...
        if available_slots > 0:
            ...
            for sym, df in data.items():
                ...
                rh20, rh50 = row['RH20'], row['RH50']
                if close >= rh10 and adx14 > 15 and rsi14 > 30:
                    print(f'[DEBUG] {sym} | Close: {close:.2f} RH10: {rh10:.2f} ADX14: {adx14:.2f} RSI14: {rsi14:.2f}')
                    stop = close - 2.0 * a14
                    target = rh50 + a14
                    r2r = reward_to_risk(close, target, stop)
                    if r2r < 1.0:
                        continue
                    score = ai_score(dict(breakout_ratio=close/rh20, adx14=adx14, rsi14=rsi14, r2r=r2r))
            ...
            for score, sym, price, stop in candidates[:available_slots]:
                ...
                if cash >= cost:
                    ...
                    cash -= cost
                    trades.append(dict(date=dt, symbol=sym, side='BUY', qty=qty, price=round(price,2), reason='ENTRY'))

    equity = []
    for dt in dates:
        mtm = sum(p.qty * (data[sym].at[dt,'Close'] if dt in data[sym].index else p.last_close) for sym, p in positions.items())
...
def main():
    ...
    for sym in symbols:
        try:
            ...
            d['ATR14'] = atr(d,14)
            d['ADX14'] = adx(d,14)
            d['RH10'] = d['Close'].rolling(10).max()
d['RH20'] = d['Close'].rolling(20).max()
            d['RH50'] = d['Close'].rolling(50).max()
    ...
    print(f"Backtest complete. {len(trades)} trades. Saved to CSV.")
